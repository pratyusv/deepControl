...
pose_pub = rospy.Publisher(model+'/command/pose', PoseStamped, queue_size=100)
rpyt_sub = rospy.Subscriber(model + '/command/roll_pitch_yawrate_thrust', RollPitchYawrateThrust, rpyth_cb, queue_size=100)
...
while not rospy.is_shutdown():
	...
	if abs(curr_pos[0] - target[0]) < 0.1 and  abs(curr_pos[1] - target[1]) < 0.1 and abs(curr_pos[2] - target[2]) < 0.1:
		sample_x = np.random.uniform(low=-2. , high=2.)
		sample_y = np.random.uniform(low=-2.0, high=2.0)
		sample_z = np.random.uniform(low=0.4, high=2.0)


		if abs(sample_x - curr_pos[0]) < 1.5 and abs(sample_y -curr_pos[1]) < 1.5 :
			counter += 1
...
np.save(data_dir + 'firefly_input_state.npy',state_)
np.save(data_dir + 'firefly_setpoints.npy', setpoints_)
np.save(data_dir + 'firefly_controls.npy'   ,controls_)
...
ax2[0].plot(roll_sp)
ax2[1].plot(pitch_sp)
ax2[2].plot(yaw_rate_sp)
ax2[3].plot(thrust_sp)
